...
fun! HtmlIndentGetter(lnum)
    ...
    let php = '<?php'
    if   0 < searchpair(php, '', '?>', 'nWb')
    \ && 0 < searchpair(php, '', '?>', 'nW')
      let phpline = search(php, 'nWb')
      if getline(a:lnum) =~ '?>'
        return indent(phpline)
      endif
      if getline(lnum) !~ php && getline(a:lnum) !~ '?>'
        if restore_ic == 0
          setlocal noic
        endif   
      endif
      if getline(phpline) =~ php . '\s*\S'
        return GetPhpIndent()
      elseif 0 == indent(phpline)
        return GetPhpIndent()
      else
        return GetPhpIndent() + indent(phpline) + &sw
      endif
    endif
